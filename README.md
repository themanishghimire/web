...
- JSON-LD structured data: `ProfessionalService` + `Person` schema on the homepage,
  `Person` schema on About, `Service` schema (with an offer catalog of all 8 services)
  on Services
- `robots.txt` + `sitemap.xml` for crawlability
- Canonical tags on every page
- Semantic heading hierarchy (one `h1` per page) and descriptive `alt` text on images
